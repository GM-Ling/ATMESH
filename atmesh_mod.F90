...
!++
      ...
      ELSEIF ( TRIM(atm_grd) == 'structured' ) THEN
        ...
        if(.not. allocated(atm_strucgrd%ICEC)) then
            allocate (atm_strucgrd%ICEC (1:nlon,1:nlat,1))
        endif
...
!++ GML
        if (atmice .eq. 1)then
        call check( nf90_get_var(ncid,ICEC_varid, atm_strucgrd%ICEC, start3D, count3D) ) ! GML added 20210727
        endif
        write(info,*) subname,' atmice= ', atmice
        call ESMF_LogWrite(info, ESMF_LOGMSG_INFO, rc=rc)
!++
